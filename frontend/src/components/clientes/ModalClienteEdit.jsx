...
import { crearCliente, consultarRENIEC, actualizarCliente } from "../../services/clienteService";
...
function ModalClienteEdit({ onClose, onClienteActualizado, cliente }) {
  ...
  useEffect(() => {
    if (cliente) {
      ...
      if (cliente.departamento) setDepartamentoSeleccionado(cliente.departamento);
      if (cliente.provincia) setProvinciaSeleccionada(cliente.provincia);
      if (cliente.distrito) setDistritoSeleccionado(cliente.distrito);
    }
  }, [cliente]);
  ...
  const handleInputChange = (e) => {
    ...
    if (name === "numeroDocumento") {
      if (value !== clienteData.numeroDocumento) {
        ...
        if (tipoDetectado !== clienteData.tipoDocumento) {
          setClienteData((prev) => ({ ...prev, tipoDocumento: tipoDetectado }));
        }

        setTimeout(() => consultarAutomaticamente(tipoDetectado, value), 500);
      } else {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (cliente) {
        ...
        response = await actualizarCliente(cliente.id, datosParaEnviar);
      ...
      const clienteRespuesta = response.cliente || response;
      onClienteActualizado(clienteRespuesta);
            ...
            >
              ...
              <div>
                ...
                <input
                  ...
                  placeholder={`Ingrese documento de ${getMaxLength(clienteData.tipoDocumento)} dígitos`}
                  style={{
                    width: "100%",
                    padding: "8px",
                    border: "1px solid #ddd",
