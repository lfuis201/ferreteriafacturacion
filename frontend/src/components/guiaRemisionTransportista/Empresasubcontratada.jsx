...
import { crearPagadorFlete, consultarRENIEC } from "../../services/pagadorFleteService";

// Componente principal del modal pagador de flete
function Empresasubcontratada({ onClose, onPagadorFleteCreado }) {
  ...
  const [pagadorFleteData, setPagadorFleteData] = useState({
    nombre: "",
    tipoDocumento: "DNI",
    numeroDocumento: "",
    telefono: "",
  ...
  const handleUbigeoCreado = (nuevoUbigeo) => {
    // Actualizar el pagador de flete con el nuevo ubigeo seleccionado
    setPagadorFleteData((prev) => ({
      ...prev,
  ...
  const detectarTipoDocumento = (numeroDocumento) => {
    ...
    if (numeroLimpio.length === 8) {
      ...
      return tiposde15.includes(pagadorFleteData.tipoDocumento) ? pagadorFleteData.tipoDocumento : "CE";
    }
    return pagadorFleteData.tipoDocumento; // Mantener el tipo actual si no coincide
  };
  ...
  const handleInputChange = (e) => {
    ...
    if (name === "numeroDocumento") {
      ...
      if (tipoDetectado !== pagadorFleteData.tipoDocumento) {
        updatedData.tipoDocumento = tipoDetectado;
      }
    }

    setPagadorFleteData(updatedData);
    ...
    if (name === "tipoDocumento") {
      consultarAutomaticamente(value, pagadorFleteData.numeroDocumento);
    }
  };
  ...
  const consultarRENIECManual = async () => {
    if (!pagadorFleteData.numeroDocumento || !pagadorFleteData.tipoDocumento) {
      alert("Debe ingresar el tipo y número de documento");
      return;
    }

    if (pagadorFleteData.tipoDocumento !== "DNI" && pagadorFleteData.tipoDocumento !== "RUC") {
    ...
    try {
      ...
      const response = await consultarRENIEC(pagadorFleteData.tipoDocumento, pagadorFleteData.numeroDocumento);
      ...
      if (response.datos) {
        ...
        setPagadorFleteData(prev => ({
          ...prev,
          nombre: prev.nombre || (pagadorFleteData.tipoDocumento === "DNI" 
            ? `${datos.nombres || ""} ${datos.apellidoPaterno || ""} ${datos.apellidoMaterno || ""}`.trim()
            : datos.razonSocial || datos.nombre || ""),
        ...
        setMensajeRENIEC(
          `✅ Datos obtenidos de ${pagadorFleteData.tipoDocumento === "DNI" ? "RENIEC" : "SUNAT"} exitosamente`
        );
      } else {
      ...
      setMensajeRENIEC(
        `❌ Error al consultar ${pagadorFleteData.tipoDocumento === "DNI" ? "RENIEC" : "SUNAT"}: ${error.message}`
      );
    } finally {
      setConsultandoRENIEC(false);
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const pagadorFleteParaCrear = {
        tipoDocumento: pagadorFleteData.tipoDocumento,
        numeroDocumento: pagadorFleteData.numeroDocumento,
        nombre: pagadorFleteData.nombre,
        telefono: pagadorFleteData.telefono,
        direccion: pagadorFleteData.direccion,
        departamento: pagadorFleteData.departamento,
        provincia: pagadorFleteData.provincia,
        distrito: pagadorFleteData.distrito,
        ubigeo: pagadorFleteData.ubigeo,
        consultarRENIEC: pagadorFleteData.tipoDocumento === 'DNI' || pagadorFleteData.tipoDocumento === 'RUC'
      };

      const response = await crearPagadorFlete(pagadorFleteParaCrear);
      
      if (response.pagadorFlete) {
        onPagadorFleteCreado(response.pagadorFlete);
        onClose();
      }
    } catch (error) {
      console.error('Error al crear pagador de flete:', error);
      alert(`Error al crear pagador de flete: ${error.message}`);
    } finally {
      setLoading(false);
    }
  };
          ...
          <form onSubmit={handleSubmit} style={{ padding: "20px" }}>
            ...
            >
              ...
              <div>
                ...
                <select
                  name="tipoDocumento"
                  value={pagadorFleteData.tipoDocumento}
              ...
              <div>
                ...
                <input
                  ...
                  value={pagadorFleteData.numeroDocumento}
                  onChange={handleInputChange}
                  required
                  maxLength={getMaxLength(pagadorFleteData.tipoDocumento)}
                  placeholder={
                    pagadorFleteData.tipoDocumento === "DNI"
                      ? "8 dígitos - Consulta automática"
                      : pagadorFleteData.tipoDocumento === "RUC"
                      ? "11 dígitos - Consulta automática"
                      : `${getMaxLength(pagadorFleteData.tipoDocumento)} dígitos`
                  }
                  style={{
                    width: "100%",
                    padding: "8px",
                ...
                <input
                  ...
                  placeholder={
                     pagadorFleteData.tipoDocumento === "DNI"
                       ...
                       : "Nombre completo del pagador de flete"
                   }
                  style={{
                    width: "100%",
                    padding: "8px",
              ...
              <div style={{ gridColumn: "1 / -1" }}>
                ...
                {pagadorFleteData.ubigeo && (
                  ...
                  >
                    ✅ Ubigeo seleccionado: {pagadorFleteData.ubigeo} - {pagadorFleteData.departamento}, {pagadorFleteData.provincia}, {pagadorFleteData.distrito}
                  </div>
                )}
              </div>
              ...
              <div style={{ gridColumn: "1 / -1" }}>
                ...
                <input
                  type="text"
                  name="direccion"
                  value={pagadorFleteData.direccion}
                  onChange={handleInputChange}
              ...
              {(pagadorFleteData.tipoDocumento === "DNI" ||
                pagadorFleteData.tipoDocumento === "RUC") &&
                pagadorFleteData.numeroDocumento && (
                  <div style={{ gridColumn: "1 / -1" }}>
                    <button
                      type="button"
                    ...
                    >
                      Consulta manual opcional. Al crear el pagador de flete se consulta automáticamente.
                    </small>
                  </div>
                )}
            ...
            >
              ...
              >
                {loading ? "Creando..." : "Crear Pagador de Flete"}
              </button>
            </div>
          </form>
        </div>
...
export default Empresasubcontratada;