...
function ProductoDetalle({ onProductoSeleccionado, productos: productosProps }) {
  ...
  const handleInputChange = (e, section) => {
    ...
    if (section === 'producto') {
      ...
      const nuevoValor = name === 'cantidad' || name === 'precioUnitario' || name === 'precioCompra'
        ? parseFloat(value) || 0
        : value;
      setInventario(prev => {
        const actualizado = { ...prev, [name]: nuevoValor };
        // Recalcular total cuando cambian cantidad o precioUnitario
        if (name === 'cantidad' || name === 'precioUnitario') {
          actualizado.total = parseFloat(((actualizado.cantidad || 0) * (actualizado.precioUnitario || 0)).toFixed(2));
        }
        return actualizado;
      });
    }
  ...
  const confirmarSeleccionProducto = (producto) => {
    if (onProductoSeleccionado) {
      // Pasar precio y cantidad seleccionados al formulario de ventas
      const productoParaVenta = {
        ...producto,
        precioVenta: parseFloat(inventario.precioUnitario || producto.precioVenta || 0),
        cantidadSeleccionada: parseFloat(inventario.cantidad || 1),
        unidadSeleccionada: unidadSeleccionada || 'PUBLICO'
      };
      onProductoSeleccionado(productoParaVenta);
    }
  };
  ...
  const cargarPresentaciones = async (productoId) => {
    try {
      ...
      // Definir descripción (PUBLICO/X MAYOR) y precio por defecto si existen presentaciones
      if (presentacionesFormateadas && presentacionesFormateadas.length > 0) {
        const publico = presentacionesFormateadas.find(p => (p.descripcion || '').toUpperCase() === 'PUBLICO');
        const xMayor = presentacionesFormateadas.find(p => (p.descripcion || '').toUpperCase() === 'X MAYOR');
        const seleccionInicial = publico || xMayor || presentacionesFormateadas[0];

        const unidadInicial = (seleccionInicial.descripcion || seleccionInicial.unidad || 'PUBLICO');
        let precioInicial = parseFloat(seleccionInicial.precio1 || 0);
        if ((unidadInicial || '').toUpperCase() === 'X MAYOR') {
          precioInicial = parseFloat(seleccionInicial.precio2 || seleccionInicial.precio1 || 0);
        }

        setUnidadSeleccionada(unidadInicial);
        setInventario(prev => ({
          ...prev,
          precioUnitario: precioInicial,
          total: parseFloat(((prev.cantidad || 1) * (precioInicial || 0)).toFixed(2))
        }));
      } else {
        // Sin presentaciones: usar precioVenta del producto seleccionado y unidad 'PUBLICO'
        setUnidadSeleccionada('PUBLICO');
        setInventario(prev => ({
          ...prev,
          precioUnitario: parseFloat(productoSeleccionado?.precioVenta || prev.precioUnitario || 0),
          total: parseFloat(((prev.cantidad || 1) * (parseFloat(productoSeleccionado?.precioVenta || prev.precioUnitario || 0))).toFixed(2))
        }));
      }
    } catch (error) {
  ...
  const handleUnidadMedidaChange = (e) => {
    const unidadODescripcion = e.target.value;
    setUnidadSeleccionada(unidadODescripcion);
    const presentacion = presentaciones.find(p => p.descripcion === unidadODescripcion || p.unidad === unidadODescripcion);
    let precio;
    if (presentacion) {
      const desc = (presentacion.descripcion || '').toUpperCase();
      if (desc === 'PUBLICO') {
        precio = parseFloat(presentacion.precio1 || 0);
      } else if (desc === 'X MAYOR') {
        precio = parseFloat(presentacion.precio2 || presentacion.precio1 || 0);
      } else {
        // Por defecto usar precio1
        precio = parseFloat(presentacion.precio1 || 0);
      }
    } else {
      // Fallback sin presentaciones: mapear PUBLICO/X MAYOR desde campos del producto
      if (unidadODescripcion === 'PUBLICO') {
        precio = parseFloat(productoSeleccionado?.precioVenta || inventario.precioUnitario || 0);
      } else if (unidadODescripcion === 'X MAYOR') {
        precio = parseFloat(
          productoSeleccionado?.presentacion_precio2 ||
          productoSeleccionado?.precio2 ||
          productoSeleccionado?.precioMayor ||
          productoSeleccionado?.precioMayorista ||
          inventario.precioUnitario || 0
        );
      } else {
        precio = inventario.precioUnitario;
      }
    }
    setInventario(prev => ({
      ...prev,
      precioUnitario: precio,
      total: parseFloat(((prev.cantidad || 1) * (precio || 0)).toFixed(2))
    }));
  };
          ...
          <div>
            {productoSeleccionado && (
              <>
                ...
                {/* Módulo de cantidad, unidad, precios y total */}
                <div style={{
                  marginTop: '20px',
                  padding: '15px',
                  backgroundColor: '#f8f9fa',
                  ...
                  <div style={{
                    display: 'grid',
                    gridTemplateColumns: '120px 160px 200px 180px 160px',
                    gap: '12px',
                    alignItems: 'end'
                  }}>
                    {/* Cantidad */}
                    <div>
                      <label style={{ display: 'block', marginBottom: '5px', fontSize: '10px', fontWeight: 'bold' }}>Cantidad</label>
                      <input
                        type="number"
                        min="1"
                        step="1"
                    ...
                    {/* Precio de compra */}
                    <div>
                      <label style={{ display: 'block', marginBottom: '5px', fontSize: '10px', fontWeight: 'bold' }}>Precio de compra</label>
                      <input
                        type="number"
                        min="0"
                        step="0.01"
                    ...
                    {/* Unidad de medida */}
                    <div>
                      <label style={{ display: 'block', marginBottom: '5px', fontSize: '10px', fontWeight: 'bold' }}>Unidad de medida</label>
                      <select
                      ...
                      >
                        {presentaciones.length > 0 ? (
                          presentaciones.map((p, idx) => (
                            <option key={idx} value={p.descripcion}>{p.descripcion}</option>
                          ))
                        ) : (
                          <>
                            <option value="PUBLICO">PUBLICO</option>
                            <option value="X MAYOR">X MAYOR</option>
                          </>
                        )}
                      </select>
                    </div>
                    ...
                    <div>
                      <label style={{ display: 'block', marginBottom: '5px', fontSize: '10px', fontWeight: 'bold' }}>Precio unitario de venta</label>
                      <input
                        type="number"
                        min="0"
                        step="0.01"
                    ...
                    {/* Total */}
                    <div>
                      <label style={{ display: 'block', marginBottom: '5px', fontSize: '10px', fontWeight: 'bold' }}>Total</label>
                      <input
                        type="number"
                        readOnly
                        name="total"
