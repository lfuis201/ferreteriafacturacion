...
import { crearConductor, consultarRENIEC, actualizarConductor } from "../../services/conductorService";
import { consultarSUNAT } from "../../services/transportistaService";

// Componente principal del modal conductor para transporte público
function conductores ({ onClose, onConductorCreado, initialData, onConductorActualizado }) {
  ...
  useEffect(() => {
    if (initialData) {
      setConductorData(prev => ({
        ...
        numeroDocumento: initialData.numeroDocumento || initialData.numero || "",
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (initialData?.id) {
        // Actualizar
        const response = await actualizarConductor(initialData.id, conductorData);
        if (response.conductor) {
          onConductorActualizado && onConductorActualizado(response.conductor);
          onClose();
        }
      } else {
        // Crear
    ...
    <>
      ...
      >
        ...
        >
          ...
          >
            <h3 style={{ margin: 0, fontSize: "16px" }}>{initialData ? 'Editar Conductor' : 'Nuevo Conductor - Transporte Público'}</h3>
            <button
              type="button"
              onClick={onClose}
              style={{
