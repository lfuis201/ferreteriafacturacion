...
const ListaVentas = ({ onVerDetalle }) => {
  ...
  const getStatusBadge = (status) => {
    const base = "inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-tight ";
  ...
  const getSunatBadge = (status) => {
    const base = "inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-tight ";
    switch (status?.toUpperCase()) {
    ...
    <div className="flex flex-col space-y-6 p-4 md:p-6 bg-slate-50/30">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="text-2xl font-bold tracking-tight text-menta-petroleo">Listado de Comprobantes</h2>
            <p className="text-sm text-slate-500">Facturación electrónica y gestión de ventas</p>
          </div>
        </div>
        <div className="flex items-center gap-3">
          <button
            onClick={() => cargarVentas()}
            className="inline-flex h-10 items-center gap-2 rounded-lg border border-slate-200 bg-white px-4 py-2 text-sm font-semibold text-slate-600 transition hover:bg-slate-50 shadow-sm"
          >
          ...
          <button
            ...
            className="inline-flex h-10 items-center justify-center gap-2 rounded-lg bg-menta-petroleo px-5 text-sm font-bold text-white shadow-lg shadow-menta-petroleo/20 transition hover:bg-menta-marino active:scale-95"
          >
            <FileText size={18} />
            NUEVA VENTA
          </button>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
          <p className="text-[10px] font-bold uppercase tracking-wider text-slate-400">Total Facturado</p>
          <p className="text-2xl font-black text-menta-marino">
            {formatearMoneda(ventas.filter(v => v.estado !== 'ANULADA').reduce((s, v) => s + (parseFloat(v.total) || 0), 0))}
          </p>
        </div>
        <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
          <p className="text-[10px] font-bold uppercase tracking-wider text-slate-400">Comprobantes</p>
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <p className="text-2xl font-black text-red-500">{ventas.filter(v => v.estado === 'ANULADA').length}</p>
        </div>
      </div>
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
        <div className="mb-4 flex items-center justify-between border-b pb-3">
          ...
          <div className="flex items-center gap-4">
            <button
              ...
              className="group flex items-center gap-2 text-xs font-semibold text-slate-400 hover:text-menta-petroleo transition"
            >
        ...
        {mostrarColumnas && (
          <div className="mb-6 grid grid-cols-2 gap-3 rounded-xl bg-slate-50 p-4 sm:grid-cols-3 lg:grid-cols-6 animate-in fade-in slide-in-from-top-2 duration-200">
            {Object.entries(columnLabels).map(([key, label]) => (
              <label key={key} className="flex cursor-pointer items-center gap-2 text-[11px] font-semibold text-slate-600 hover:text-menta-petroleo transition">
                <input
                  type="checkbox"
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-4 lg:grid-cols-4">
          <div className="space-y-1.5">
            ...
            <div className="relative">
              ...
              <input
                type="date"
                name="fechaInicio"
                className="w-full rounded-lg border border-slate-200 bg-slate-50/50 py-2 pl-10 pr-3 text-sm focus:border-menta-turquesa focus:ring-2 focus:ring-menta-turquesa outline-none transition"
          ...
          <div className="space-y-1.5">
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full rounded-lg border border-slate-200 bg-slate-50/50 py-2 pl-10 pr-3 text-sm font-semibold focus:border-menta-turquesa outline-none transition"
          ...
          <div className="space-y-1.5">
            ...
            <select
              name="estado"
              className="w-full rounded-lg border border-slate-200 bg-slate-50/50 py-2 px-3 text-sm focus:border-menta-turquesa outline-none transition"
      ...
      <div className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
          ...
          <table className="w-full text-left text-sm whitespace-nowrap">
            <thead className="bg-slate-50 text-[10px] font-bold uppercase tracking-widest text-slate-400">
              <tr>
                ...
                {columnasVisibles.id && <th className="px-5 py-4">ID</th>}
                {columnasVisibles.fechaVenta && <th className="px-5 py-4">Fecha</th>}
                {columnasVisibles.comprobante && <th className="px-5 py-4">Comprobante</th>}
                {columnasVisibles.cliente && <th className="px-5 py-4">Cliente</th>}
                {columnasVisibles.estado && <th className="px-5 py-4 text-center">Estado</th>}
                {columnasVisibles.estadoSunat && <th className="px-5 py-4 text-center">SUNAT</th>}
                {columnasVisibles.total && <th className="px-5 py-4 text-right">Total</th>}
                {columnasVisibles.acciones && <th className="px-5 py-4 text-center">Acciones</th>}
              </tr>
            </thead>
                ...
                <tr>
                  <td colSpan="20" className="py-24 text-center">
                    <div className="flex flex-col items-center gap-3 text-slate-300">
                      ...
                      <p className="text-sm font-bold uppercase tracking-wider">No se encontraron ventas</p>
                    </div>
                  </td>
                </tr>
              ) : (
                ventas.map((venta, index) => (
                  <tr key={venta.id} className="group transition hover:bg-slate-50/50">
                    <td className="px-5 py-4 text-center text-slate-300 font-bold">
                      {index + 1}
                    </td>
                    {columnasVisibles.id && <td className="px-5 py-4 text-slate-400">#{venta.id}</td>}
                    {columnasVisibles.fechaVenta && (
                      <td className="px-5 py-4 font-semibold text-slate-600">
                        {formatearFecha(venta.fechaVenta || venta.createdAt)}
                      </td>
                    )}
                    {columnasVisibles.comprobante && (
                      <td className="px-5 py-4">
                        <div className="flex items-center gap-2">
                          <span className="font-bold text-menta-petroleo">{venta.serieComprobante}</span>
                          <span className="text-slate-300">-</span>
                          <span className="text-slate-700">{venta.numeroComprobante}</span>
                        </div>
                    ...
                    {columnasVisibles.cliente && (
                      <td className="px-5 py-4">
                        <div className="flex flex-col">
                          <span className="max-w-[200px] truncate font-bold text-slate-700">{venta.Cliente?.nombre || venta.clienteNombre || '-'}</span>
                          <span className="text-[10px] text-slate-400 uppercase tracking-tighter">{venta.Cliente?.numeroDocumento}</span>
                        </div>
                      </td>
                    )}
                    ...
                    {columnasVisibles.acciones && (
                      <td className="px-5 py-4 text-center">
                        <div className="flex justify-center items-center gap-1">
                          <button
                            className="rounded-lg p-2 text-slate-400 transition hover:bg-slate-100 hover:text-menta-turquesa"
                          ...
                          <div className="relative" data-pdf-dropdown>
                            ...
                            {openPdfDropdownId === venta.id && (
                              <div className="absolute right-0 top-full z-10 mt-1 min-w-[140px] rounded-xl border border-slate-200 bg-white p-2 shadow-xl animate-in fade-in zoom-in-95">
                          ...
                          <div className="group/menu relative">
                            ...
                            <div className="invisible absolute right-0 top-full z-10 mt-1 min-w-[160px] rounded-xl border border-slate-200 bg-white p-2 shadow-2xl group-hover/menu:visible animate-in fade-in zoom-in-95">
                              <button onClick={() => handleConsultarSunat(venta.id, venta.numeroComprobante)} className="flex w-full items-center gap-3 rounded-lg px-4 py-2 text-left text-[11px] font-bold uppercase text-slate-600 hover:bg-emerald-50 hover:text-emerald-600 transition">
                                <RefreshCcw size={14} /> Estado SUNAT
                              </button>
                              <button onClick={() => handleReenviarSunat(venta.id, venta.numeroComprobante)} className="flex w-full items-center gap-3 rounded-lg px-4 py-2 text-left text-[11px] font-bold uppercase text-slate-600 hover:bg-menta-suave hover:text-menta-petroleo transition">
                                <Send size={14} /> Reenviar SUNAT
                              </button>
                              <button onClick={() => handleGenerarGuia(venta)} className="flex w-full items-center gap-3 rounded-lg px-4 py-2 text-left text-[11px] font-bold uppercase text-slate-600 hover:bg-blue-50 hover:text-blue-600 transition">
                                <Truck size={14} /> Generar Guía
                              </button>
                              <button onClick={() => handleEnviarWhatsApp(venta)} className="flex w-full items-center gap-3 rounded-lg px-4 py-2 text-left text-[11px] font-bold uppercase text-slate-600 hover:bg-emerald-50 hover:text-emerald-600 transition">
                                <Share2 size={14} /> WhatsApp
                              </button>
                              <div className="my-1 border-t border-slate-100" />
                              <button onClick={() => handleAnular(venta.id, venta.numeroComprobante)} className="flex w-full items-center gap-3 rounded-lg px-4 py-2 text-left text-[11px] font-bold uppercase text-red-400 hover:bg-red-50 hover:text-red-500 transition">
        ...
        {/* Pagination Section */}
        <div className="flex flex-col items-center justify-between gap-4 border-t border-slate-100 bg-slate-50/50 px-8 py-5 sm:flex-row">
          <p className="text-sm font-medium text-slate-400">
            Mostrando <span className="font-black text-slate-600">{ventas.length}</span> registros
          </p>
          <div className="flex items-center gap-1.5">
            <button className="flex h-9 w-9 items-center justify-center rounded-xl border border-slate-200 bg-white text-slate-400 transition hover:bg-slate-50 disabled:opacity-30">
              <ChevronFirst size={18} />
            </button>
            <button className="flex h-9 w-9 items-center justify-center rounded-xl border border-slate-200 bg-white text-slate-400 transition hover:bg-slate-50 disabled:opacity-30">
              <ChevronLeft size={18} />
            </button>
            <div className="flex h-9 min-w-[36px] items-center justify-center rounded-xl bg-menta-petroleo px-3 text-sm font-bold text-white shadow-lg shadow-menta-petroleo/20">
              1
            </div>
            <button className="flex h-9 w-9 items-center justify-center rounded-xl border border-slate-200 bg-white text-slate-400 transition hover:bg-slate-50 disabled:opacity-30">
